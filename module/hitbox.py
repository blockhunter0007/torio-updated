...
class HitboxController:
    def __init__(self, pm: pymem.Pymem = None, version_config: dict = None):
        ...
        self.series = self.version_config.get('series', '1.21.13')
        self.hitbox_pattern = self.version_config.get('hitbox_pattern', b'\xF3\x0F\x10\x40\x18\x48\x83\xC4\x20')
        self.shadow_pattern = self.version_config.get('shadow_pattern')  # Noneになる場合あり（1004, 1101など）
        
        self.shadow_patch_type = self.version_config.get('shadow_patch_type', 'default')
        self.hitbox_patch_length = self.version_config.get('hitbox_patch_length', 5)
        self.shadow_patch_length = self.version_config.get('shadow_patch_length', 6)
        self.hitbox_patch_type = self.version_config.get('hitbox_patch_type', 'default')
        self.shadow_value_on = self.version_config.get('shadow_value_on', 0.6)
        self.shadow_value_off = self.version_config.get('shadow_value_off', 0.6)
    ...
    def set_version_config(self, version_config: dict):
        if version_config:
            ...
            self.series = version_config.get('series', '1.21.13')
            self.hitbox_pattern = version_config.get('hitbox_pattern', self.hitbox_pattern)
            self.shadow_pattern = version_config.get('shadow_pattern')
            self.shadow_patch_type = version_config.get('shadow_patch_type', 'default')
            self.hitbox_patch_length = version_config.get('hitbox_patch_length', self.hitbox_patch_length)
            self.hitbox_patch_type = version_config.get('hitbox_patch_type', 'default')
            self.shadow_value_on = version_config.get('shadow_value_on', 0.6)
            self.shadow_value_off = version_config.get('shadow_value_off', 0.6)
    ...
    def validate_address(self):
        if not self.hitbox_addr or not self.initialized:
            return False
        if not self.validate_process():
            ...
            return False
        try:
            self.pm.read_bytes(self.hitbox_addr, self.hitbox_patch_length)
            if self.shadow_addr:
                self.pm.read_bytes(self.shadow_addr, self.shadow_patch_length)
            return True
        except Exception:
            self.initialized = False
            self.hitbox_addr = None
    ...
    def find_hitbox_addresses(self, retries=3, delay=1.0):
        ...
        for attempt in range(retries):
            try:
                ...
                print(f"Hitbox: Initialized at 0x{self.hitbox_addr:X}")
                if self.shadow_addr:
                    print(f"Shadow: at 0x{self.shadow_addr:X}")
                else:
                    print("Shadow: Skipped (not required for this version)")
    ...
    def _write_shadow_patch(self):
        ...
        try:
            ...
            return_address = self.shadow_addr + self.shadow_patch_length
            ...
            try:
                ...
                self.WriteProcessMemory(self.process_handle, ctypes.c_void_p(self.shadow_newmem),
            ...
            self.VirtualProtectEx(self.process_handle, ctypes.c_void_p(self.shadow_addr), self.shadow_patch_length,
                                  self.PAGE_EXECUTE_READWRITE, ctypes.byref(old_protect))
            try:
                self.pm.write_bytes(self.shadow_addr, patch_bytes, len(patch_bytes))
            except Exception:
                bytes_written = ctypes.c_size_t()
                self.WriteProcessMemory(self.process_handle, ctypes.c_void_p(self.shadow_addr),
    ...
    def _build_hitbox_shellcode(self, value: float) -> bytes:
        shellcode = b'\xC7\x40\x18' + struct.pack('<f', value)
        shellcode += b'\xF3\x0F\x10\x40\x18'
        return shellcode
    ...
    def _write_hitbox_to_memory(self, value: float):
        if not self.hitbox_addr or not self.hitbox_newmem:
            return False
        try:
            shellcode = self._build_hitbox_shellcode(value)

            return_address = self.hitbox_addr + self.hitbox_patch_length
            ...
            try:
                ...
                bytes_written = ctypes.c_size_t()
                self.WriteProcessMemory(self.process_handle, ctypes.c_void_p(self.hitbox_newmem),
                                        shellcode, len(shellcode), ctypes.byref(bytes_written))

            jmp_offset = self.hitbox_newmem - (self.hitbox_addr + 5)
            patch_bytes = b'\xE9' + struct.pack('<i', jmp_offset)

            if self.hitbox_patch_length > 5:
                patch_bytes += b'\x90' * (self.hitbox_patch_length - 5)

            old_protect = wintypes.DWORD()
            self.VirtualProtectEx(self.process_handle, ctypes.c_void_p(self.hitbox_addr), self.hitbox_patch_length,
                                  self.PAGE_EXECUTE_READWRITE, ctypes.byref(old_protect))
            try:
                self.pm.write_bytes(self.hitbox_addr, patch_bytes, len(patch_bytes))
            except Exception:
                bytes_written = ctypes.c_size_t()
                self.WriteProcessMemory(self.process_handle, ctypes.c_void_p(self.hitbox_addr),
                                        patch_bytes, len(patch_bytes), ctypes.byref(bytes_written))
            self.VirtualProtectEx(self.process_handle, ctypes.c_void_p(self.hitbox_addr), self.hitbox_patch_length,
                                  old_protect.value, ctypes.byref(old_protect))

            self.update_status(f"Active ({self.current_hitbox:.2f}x → {value:.2f})", '#00e676')
            return True
        except Exception as e:
            self.update_status(f"Write Error: {e.__class__.__name__}", '#ff5252')
            return False

    def _write_hitbox_value_direct(self, value: float):
        if not self.hitbox_newmem:
            return False
        try:
            float_bytes = struct.pack('<f', value)
            try:
                self.pm.write_bytes(self.hitbox_newmem + 3, float_bytes, 4)
            except Exception:
                bytes_written = ctypes.c_size_t()
                self.WriteProcessMemory(self.process_handle, ctypes.c_void_p(self.hitbox_newmem + 3),
                                        float_bytes, 4, ctypes.byref(bytes_written))
            return True
        except Exception:
            return False

    def _write_hitbox_restore_value(self, value: float = 0.6):
        self._write_hitbox_value_direct(value)
        time.sleep(0.03)
    ...
    def set_hitbox_value(self, value):
        ...
        if abs(new_multiplier - self.current_hitbox) < 0.001:
            return True
            
        self.current_hitbox = new_multiplier
        
        if self.is_active and self.initialized and self.hitbox_newmem:
            actual_value = self.original_hitbox * self.current_hitbox
            success = self._write_hitbox_value_direct(actual_value)
            
            if success:
                self.update_status(f"Active ({self.current_hitbox:.2f}x → {actual_value:.2f})", '#00e676')
            else:
                self.update_status("Hitbox Write Failed", '#ff5252')
            return success
    ...
    def enable_hitbox(self):
        ...
        try:
            if self.shadow_pattern and not self.shadow_patched:
    ...
    def disable_hitbox(self):
        if not self.initialized or not self.is_active:
            return True
        try:
            self._write_hitbox_restore_value(0.6)
    ...
    def _restore_hitbox_original(self):
        ...
        try:
            ...
            try:
                ...
                bytes_written = ctypes.c_size_t()
                self.WriteProcessMemory(self.process_handle, ctypes.c_void_p(self.hitbox_addr),
                                        self.hitbox_original_bytes, len(self.hitbox_original_bytes),
    ...
    def _restore_shadow_original(self):
        ...
        try:
            ...
            try:
                ...
                bytes_written = ctypes.c_size_t()
                self.WriteProcessMemory(self.process_handle, ctypes.c_void_p(self.shadow_addr),
                                        self.shadow_original_bytes, len(self.shadow_original_bytes),
                                        ctypes.byref(bytes_written))
            self.VirtualProtectEx(self.process_handle, ctypes.c_void_p(self.shadow_addr), self.shadow_patch_length,
                                  old_protect.value, ctypes.byref(old_protect))
            self.shadow_patched = False
