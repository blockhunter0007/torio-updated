...
class ZoomController:
    def __init__(self):
        ...
        self.smooth_transition = self.config_manager.get_setting("zoom", "smooth_animation", True)
    ...
    def set_smooth_animation(self, enabled: bool):
        self.smooth_transition = enabled
        self.config_manager.set_setting("zoom", "smooth_animation", enabled)
    ...
    def initialize(self):
        if self.initialized and self.validate_address():
            if not self.hotbar_patch_address or not self.validate_hotbar_patch_address():
                self.find_hotbar_patch_address(force_rescan=True)
            return True
        ...
        try:
            ...
            self.find_hotbar_patch_address()
            
            self.initialized = True
            
            hotbar_addr_str = f"0x{self.hotbar_patch_address:X}" if self.hotbar_patch_address else "None"
            print(f"Zoom: Initialized at 0x{self.target_address:X}, default value: {self.default_value}, hotbar address: {hotbar_addr_str}")
    ...
    def perform_memory_operation(self, target_address, start_value, end_value, duration=0.05, steps=50):
        ...
        if not self.smooth_transition:
            try:
                with self.memory_lock:
                    self.pm.write_float(target_address, end_value)
            except Exception:
                pass
            self.animation_in_progress = False
    ...
    def start(self, key=None):
        if not self.is_active:
            ...
            if self.update_queue:
                self.update_queue.put(('status_update', ('zoom', "Active", '#00e676')))
    ...
    def stop(self, save_config=True, is_app_closing=False):
        if self.is_active:
            ...
            if self.update_queue:
                self.update_queue.put(('status_update', ('zoom', "Inactive", '#b0b0b0')))
