...
class SpeedController:
    def __init__(self, pm: pymem.Pymem = None, version_config: dict = None):
        ...
        self.original_length = self.version_config.get('speed_original_length', 11)
    ...
    def set_version_config(self, version_config: dict):
        if version_config:
            ...
            self.original_length = version_config.get('speed_original_length', 11)
    ...
    def _write_speed_to_memory(self, multiplier: float):
        ...
        try:
            ...
            if self.series in ("1.21.13", "1.26.2", "1.26.101", "1.26.201", "1.26.301", "1.26.1004", "1.26.1101", "1.26.1202"):
                original_load = b'\xF3\x0F\x10\x40\x7C'
                store_bytes = b'\xF3\x0F\x11\x44\x24\x28'
            else:
                self.update_status("Unsupported series for speed hack", '#ff5252')
                return False

            shellcode = original_load
            ...
            if not -0x80000000 <= mulss_offset <= 0x7FFFFFFF:
                self.update_status("Multiplier offset too far", '#ff5252')
                return False

            shellcode += b'\xF3\x0F\x59\x05' + struct.pack('<i', mulss_offset)
            shellcode += store_bytes
            ...
            if not -0x80000000 <= jmp_back <= 0x7FFFFFFF:
                self.update_status("Jump back offset too far", '#ff5252')
                return False
