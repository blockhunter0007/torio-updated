...
class AntiKnockbackController:
    ...
    def _calc_rip_offset(self, target_addr, current_shellcode):
        next_ip = self.code_start + len(current_shellcode) + 4 + 4
        return target_addr - next_ip
    ...
    def find_pattern_and_setup_hook(self, retries=3, delay=1.0):
        ...
        for attempt in range(retries):
            try:
                ...
                self.pm.write_bytes(self.kb_z_mult_addr, self.float_to_bytes(self.kb_y_mult),  4)
                self.pm.write_bytes(self.kb_y_mult_addr, self.float_to_bytes(self.kb_xz_mult), 4)
                ...
                time.sleep(delay)
        self.update_status("Pattern scan or memory allocation failed", '#ff5252')
        return False
    ...
    def set_multipliers(self, xz_mult=None, y_mult=None):
        ...
        if changed and self.initialized:
            ...
            status_text = f"XZ: {self.kb_xz_mult:.2f}  Y: {self.kb_y_mult:.2f}"
            self.update_status(status_text, '#00e676' if self.is_active else '#b0b0b0')
    ...
    def enable_antiknockback(self):
        ...
        try:
            ...
            shellcode += b"\xF3\x0F\x59\x05" + struct.pack("<i", self._calc_rip_offset(self.kb_x_mult_addr, shellcode))
            shellcode += b"\xF3\x0F\x11\x40\x18"                       # movss [rax+0x18], xmm0

            shellcode += b"\xF3\x0F\x10\x42\x04"                       # movss xmm0, [rdx+4]
            shellcode += b"\xF3\x0F\x59\x05" + struct.pack("<i", self._calc_rip_offset(self.kb_z_mult_addr, shellcode))
            shellcode += b"\xF3\x0F\x11\x40\x1C"                       # movss [rax+0x1C], xmm0   ← Y方向

            shellcode += b"\xF3\x0F\x10\x42\x08"                       # movss xmm0, [rdx+8]
            shellcode += b"\xF3\x0F\x59\x05" + struct.pack("<i", self._calc_rip_offset(self.kb_y_mult_addr, shellcode))
            shellcode += b"\xF3\x0F\x11\x40\x20"                       # movss [rax+0x20], xmm0   ← Z方向

            jmp_back_offset = (self.hook_address + 5) - (self.code_start + len(shellcode) + 5)
            shellcode += b"\xE9" + struct.pack("<i", jmp_back_offset)

            shellcode += b"\x66\x0F\x1F\x84\x00\x00\x00\x00\x00"

            old_protect = ctypes.c_ulong()
            ctypes.windll.kernel32.VirtualProtectEx(
                self.process_handle, ctypes.c_void_p(self.code_start), len(shellcode), 0x40, ctypes.byref(old_protect)
            )
            self.pm.write_bytes(self.code_start, shellcode, len(shellcode))
            ctypes.windll.kernel32.VirtualProtectEx(
                self.process_handle, ctypes.c_void_p(self.code_start), len(shellcode), old_protect.value, ctypes.byref(ctypes.c_ulong())
            )

            nop4 = b"\x90\x90\x90\x90"
            ctypes.windll.kernel32.VirtualProtectEx(
                self.process_handle, ctypes.c_void_p(self.hook_address + 5), 4, 0x40, ctypes.byref(old_protect)
            )
            self.pm.write_bytes(self.hook_address + 5, nop4, 4)
            ctypes.windll.kernel32.VirtualProtectEx(
                self.process_handle, ctypes.c_void_p(self.hook_address + 5), 4, old_protect.value, ctypes.byref(ctypes.c_ulong())
            )

            rel_jmp = self.code_start - (self.hook_address + 5)
            patch = b"\xE9" + struct.pack("<i", rel_jmp)
            ctypes.windll.kernel32.VirtualProtectEx(
                self.process_handle, ctypes.c_void_p(self.hook_address), 5, 0x40, ctypes.byref(old_protect)
            )
            self.pm.write_bytes(self.hook_address, patch, 5)
            ctypes.windll.kernel32.VirtualProtectEx(
                self.process_handle, ctypes.c_void_p(self.hook_address), 5, old_protect.value, ctypes.byref(ctypes.c_ulong())
            )

            self.is_active = True
            self.update_status(f"Active  (XZ:{self.kb_xz_mult:.2f}  Y:{self.kb_y_mult:.2f})", '#00e676')
            return True
        except Exception as e:
            self.update_status(f"Enable failed: {str(e)}", '#ff5252')
            return False
    ...
    def disable_antiknockback(self):
        ...
        try:
            old_protect = ctypes.c_ulong()
            ctypes.windll.kernel32.VirtualProtectEx(
                self.process_handle, ctypes.c_void_p(self.hook_address), 9, 0x40, ctypes.byref(old_protect)
            )
            self.pm.write_bytes(self.hook_address, self.original_bytes, 9)
            ctypes.windll.kernel32.VirtualProtectEx(
                self.process_handle, ctypes.c_void_p(self.hook_address), 9, old_protect.value, ctypes.byref(ctypes.c_ulong())
            )
            self.is_active = False
            self.update_status("Inactive", '#b0b0b0')
            return True
        except Exception as e:
            self.update_status(f"Disable failed: {e.__class__.__name__}", '#ff5252')
            return False
